  ...
  real(kind=8),parameter::bt=0.35d0
  real(kind=8),parameter::bp=0.35d0
  real(kind=8),parameter::bx=bt+bp

  
  character(len=*), parameter :: ofile = 'Qxy035.dat'
  character(len=*), parameter :: ocfile = 'Qcxy035.dat'  
  ...
  integer,parameter::isimul=1  
  integer,parameter::iFFT=0
  integer,parameter::iAmpl=1
  integer,parameter::iVariance=1
  ...
  integer,parameter ::Ndy=500  
  real(kind=8),parameter::rdx=pi/real(Ndx)
  real(kind=8),parameter::rdy=pi/real(Ndy)
  
  integer,parameter ::Nh=16      !FFT parameter
  ...
  program main
    ...
    real(kind=8),dimension(-Ndx-1:Ndx+1)::Px       
    real(kind=8),dimension(-Ndy-1:Ndy+1)::Py  
    real(kind=8)::dxy,dxx,dmxx,dmxy
    integer::imxx,imxy
 
    real(kind=8),dimension(-Ndx+1:Ndx-1)::Pcx
    real(kind=8),dimension(-Ndy+1:Ndy-1)::Pcy 
    real(kind=8)::xip,res,gxx,gxpi,zcpx,zpcy
...
!    open(unit=34, file='fftvp.dat'))
    ...
    open(unit=43, file='xt1.dat')
    open(unit=44, file='xp1.dat')
    open(unit=45, file='xt2.dat')
    open(unit=46, file='xp2.dat')    
    open(unit=53, file='ensxt.dat')
    open(unit=54, file='ensxp.dat')

    open(unit=63, file='variance.dat') 
    open(unit=64, file='amplitude.dat')

    open(unit=71, file=ofile) 
    open(unit=81, file=ocfile) 
    ...
    do it=1,ntmax
       ...
       if(mod(it,interval)==0) then
          ...
          if(iprint==1) then
             ...
             write(43,*) time,sxt(1)
             write(44,*) time,cxp(1)
             write(45,*) time,sxt(30)
             write(46,*) time,cxp(30)          
...
if(iVariance==1.and.iAmpl==1) then
    delp=0.0d0
    delt=0.0d0
    cxtp=0.0d0
    do it=1,Nvar
       ...
       acxtp=0.0d0
       do i=1,Nc
          dxp=xp(i)-eop*real(it)
          dxt=xt(i)-eop*real(it)
          dmxp=mod(dxp,tpi)
          dmxt=mod(dxt,tpi)
          if(dmxp>pi) dmxp=dmxp-tpi
          if(dmxt>pi) dmxt=dmxt-tpi
          if(dmxp<-pi) dmxp=dmxp+tpi
          if(dmxt<-pi) dmxt=dmxt+tpi
          axp=axp+dmxp
          axt=axt+dmxt
    ...
    cxtp=cxtp/real(Nvar)
    betaB=betaD*cxtp-2.0d0
    ampB=sqrt(8*abs(betaB))
    write(63,*) betaD,delt,delp,cxtp,betaB,ampB,ampp/2.0d0
    flush(63)
    if(iprint==1) write(6,*) 'stand. dev. for \theta \phi',sqrt(delt),sqrt(delp)
    if(iprint==1) write(6,*) '<cosx>=',cxtp,'betaB=',betaB,'ampl. pred.=',ampB,'ampl. calc.=', ampp/2.0d0
endif
...
!calculating the distributions
    ...
    do it=1,Nvar
       call langevinstep(xt,xp,beta,seed) 
       do i=1,Nc
          dmxy=xp(i)-eop*real(it)
          dmxx=xt(i)-xp(i)
          dmxy=mod(dmxy,tpi)
          dmxx=mod(dmxx,tpi)
          if(dmxy>pi) dmxy=dmxy-tpi
          if(dmxx>pi) dmxx=dmxx-tpi
          if(dmxy<-pi) dmxy=dmxy+tpi
          if(dmxx<-pi) dmxx=dmxx+tpi
          imxx=Int(dmxx/rdx)
          imxy=Int(dmxy/rdy)
          if(dmxx<0) imxx=imxx-1
          if(dmxy<0) imxy=imxy-1
          if(imxx>=-Ndx-1) Px(imxx)=Px(imxx)+1.0d0
          if(imxy>=-Ndy-1) Py(imxy)=Py(imxy)+1.0d0
       enddo
    enddo
    
    do i=-Ndx+1,Ndx-1
       Px(i)=Px(i)/real(Nvar)/real(Nc)
       Py(i)=Py(i)/real(Nvar)/real(Nc)       
       write(71,*) real(i)*rdx,Px(i),Py(i)
    enddo
    flush(71)
    if(iprint==1) write(6,*) Px(0),Py(0)
    
 endif
 
 !calculating the mean-field distributions

    res=0.0d0
    call integrate_simpson(hi, 0.0d0, tpi, 10000, res)
    gxpi=( exp(tpi*op/Diffx)-1.0d0 )/res
    zpcx=0.0d0
    zcpy=0.0d0
    Pcx=0.0d0
    Pcy=0.0d0
    do i=-Ndx+1,Ndx-1
       ...
       if(i<0) xip=xip+tpi
       gxx=0.0d0
       call integrate_simpson(hi, 0.0d0, xip,10000, gxx)    
       Pcx(i)=exp((-op*xip+bx*cos(xip))/Diffx )*(1.0d0+gxx*gxpi)   
       Pcy(i)=exp(betaD*(0.5d0*ampp)*cxtp*cos(xip))
       zpcx=zpcx+Pcx(i)
       zpcy=zpcy+Pcy(i)
    enddo
    do i=-Ndx+1,Ndx-1
       Pcx(i)=Pcx(i)/zpcx
       Pcy(i)=Pcy(i)/zpcy
       write(81,*) real(i)*rdx,Pcx(i),Pcy(i)
    enddo
 ...
 !calculating the FFT spectra
   if(iFFT==1) then
      ...
      if(iprint==1) write(6,*) 'For \phi, period =',vpeakp,'log amp.=',vlogp
    endif
...
contains
  real(kind=8) function hi(x)
     use param
     real(kind=8),intent(in)::x
        hi= exp( (op*x-bx*cos(x))/Diffx )
     return
  end function hi
...
subroutine integrate_simpson(hi, a, b, n, res)
